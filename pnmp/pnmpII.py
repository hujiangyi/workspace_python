...
import numpy as np
import Color
from pnmp.utils import PreEqualizationParam, PnmpUtils
...
if not pep.isEmpty():
    print 'x1 *********************************'
    x1 = pep.toArray()
    mtc = PnmpUtils.mtc(x1)
    mtr = PnmpUtils.mtr(x1)
    amplitudes = PnmpUtils.amplitudes(x1)
    amplitudes = PnmpUtils.toArray24(amplitudes)
    print(amplitudes)
    freqResult = PnmpUtils.freqResult(x1)
...
if not pep2.isEmpty():
    print 'x2 *********************************'
    x2 = pep2.toArray()
    mtc = PnmpUtils.mtc(x2)
    mtr = PnmpUtils.mtr(x2)
    freqResult = PnmpUtils.freqResult(x2)
